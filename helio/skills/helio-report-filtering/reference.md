...
A real cohort-comparison pattern drawn from the corpus.

### A press-room site — Slicing the 7-segment press fanout

the press-room site ran the same test across 7 press segments (journalists, bloggers, vloggers, social-media marketers, etc.). The aggregate Satisfaction score combines all of them — useful as a baseline but hides which segment is least served.

The synthesis-grade workflow:

1. Apply the Segment filter to a single segment (e.g., journalists).
2. Capture a Finding for that cohort's Satisfaction read with the filter context snapshotted.
3. Clear, filter to the next segment (e.g., bloggers).
4. Capture another Finding.
5. Repeat for all 7.
6. Open the Findings panel, scope to this study, sort by Satisfaction. The spread tells you which segment the page underserves.

The headline isn't the aggregate Satisfaction. It's the segment-by-segment spread that the filter mechanic surfaces.

### Cohort comparison without a built-in side-by-side view

Helio doesn't have a saved side-by-side cohort view today. Two workarounds:

- **Sequential Findings (above).** Capture a Finding per cohort with filter snapshotted. The Findings panel becomes your side-by-side.
- **MCP `compare_segments` tool.** When an AI assistant is driving, `compare_segments` runs two filtered report calls in parallel and returns a structured diff. See `helio-mcp` for the call shape.
