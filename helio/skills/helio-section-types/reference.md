...
<!-- DERIVED FROM: 1g6Xk5gEnmmte_cRQiwYus-N2XSd-nDIILk1G9vsMpzE — Section Types v0.2 -->
...
## Worked configurations from real tests

How specific section types were configured in real Zurb tests. Use as templates when configuring new ones.

### Multiple Choice (desirability descriptor set)

**An athletic-apparel DTC homepage:** 8 descriptors split positive/negative — Helpful, Innovative, Simple, Powerful (positive) · Complicated, Confusing, Unnecessary, Uninteresting (negative). Result: positive descriptors dominated (Helpful 73%, Innovative 69%) with negatives near zero.

**A B2B data-platform homepage:** Similar descriptor set with the negative tail visible — 11% selected "Confusing," 13% "Complicated." Small but coherent minority — the finding.

### Click Test with hotspot priorities (Success metric)

**The hunting-apparel PLP Q5:** Single Primary hotspot on the filter control. Q5 graded by Primary-hotspot click rate. Result: 12% (Very Poor) — the filter affordance was effectively invisible.

**The hunting-apparel store Q1:** Same flow, but engagement metric — many hotspots, no single "right" answer. Used to read broad attention pattern, not findability.

### MaxDiff (content prioritization)

**A veteran careers landing page Q2:** Best/worst on 8 page sections — "Search veteran careers job listings," "Turn your service into a career with purpose," "Top factors to consider during your transition," etc. The MaxDiff output ranked which sections actually pull job-seekers (raw best/worst counts — not aggregated percentages, which collapse to 0%).

### Likert (Likelihood, 5-point)

**An athletic-apparel DTC homepage Q4:** "How likely would you be to purchase from this site?" Result: 33% Very Likely / 33% Somewhat Likely / 24% Neutral / 10% Unlikely. Followup: "Why did you choose that option?" (required).

### Prototype Test (Direct / Indirect / Failed grading)

**A banking-app Accounts → Detail flow:** Multi-screen prototype with expected paths through Accounts and credit-card Detail screens. Helio graded each participant as Direct Success (followed expected path), Indirect Success (reached goal but wandered), or Failed.
