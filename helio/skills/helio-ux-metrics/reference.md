...
**When you need the same metric twice.** Multi-screen tests often want `expectations` (or `comprehension`) on several screens. The platform supports this — a real running test scores three separate `expectations` metrics at 77, 78, and 78, each with its own section pair — but **the API and CLI reject the duplicate** (`Duplicate metric type "expectations"` on create, `test already has these ux metrics` on add). The validation is stricter than the product.

So when a design genuinely needs a metric more than once: **build that test in the web app**, or build the first instance via CLI/MCP and add the rest in the UI. Do *not* hand-build look-alike sections to work around it — that trades a scored, comparable metric for a raw distribution, which is the exact anti-pattern this section argues against. If you only need one scored instance, spend it on the screen whose result will drive the decision.
...
- **O2 145 V5: 92 (Very Good)** — Primary-hotspot click rate dominated the page. Healthy on its own; only meaningful in context with Intent (see below).
...
- **O2 145 V5: 87 (Good)** with positive descriptors dominant (Helpful 73%, Innovative 69%).
...
- **O2 145 V5: 38 (Poor)** while Engagement 92 / Desirability 87 — the headline cross-metric divergence. Attractive but not persuasive.
