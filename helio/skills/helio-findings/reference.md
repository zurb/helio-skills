...
## Worked findings (real example shapes)

Three findings written from real test data in the corpus. Use these as templates for how to write a finding that's specific enough to act on.

### From an athletic-apparel DTC homepage v1 (test-level finding)

> Desirability landed at 87 (Good) with positive descriptors dominant (Helpful 73%, Innovative 69%, Simple 44%, Powerful 41%) and negatives near zero. NPS came back at 14. The page produces strong positive emotional response but no share moment — the hero positions a single SKU, no community indicators, no referral hook. The 87 → 14 gap is the finding. Worth testing whether adding a "share with a teammate" element, a community/badge indicator, or a referral incentive lifts NPS without diluting Desirability. *Filter context: full audience, no segment cuts.*

### From the hunting-apparel PLP (section-level finding)

> Q5 Success metric at 12% (Very Poor) on the PLP filter. Primary-hotspot click rate this low means the filter affordance is effectively invisible from the PLP entry point. Hunters scrolling the PLP have no path to refine the catalog — they either commit to scrolling or bounce. Action: elevate the filter to a primary, sticky control. Compare against the hunting-apparel store Q1 (engagement at higher score) — attention is reaching the page; the filter just isn't reading as a filter. *Filter context: full audience.*

### From a B2B data-platform homepage (cross-question finding)

> Stated next-action (Q5) ranked "Explore the Platform" at 35% — but actual first-click (Q1) concentrated on the Gartner Magic Quadrant badge. Stated intent and behavior diverge. The Gartner proof is doing more work than the platform pitch. The next iteration should test promoting the analyst-led narrative to the hero section and see whether comprehension and intent both move. *Filter context: full audience; worth filtering by stated-intent answer for a follow-up Finding on the segment that said "Explore" but clicked Gartner.*
